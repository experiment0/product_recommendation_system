...
5. 
    ...
    Видится, что сервис для отправки метрик, допустим в prometeus, должен быть внешним \
    и не интегрироваться в данное приложение. \
    Подобный скрипт может, допустим, раз в сутки по расписанию отправлять запросы \
    по всем пользователям для подсчета метрик за предыщущий день,\
    и пересылать результаты в соответствующую систему.\
    Так подход будет более гибким и при необходимости менять систему сбора метрик,\
    нужно будет поменять ее только в одном месте, а не в каждом подобном приложении.